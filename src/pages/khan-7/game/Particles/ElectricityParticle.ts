export default class ElectricityParticle extends Phaser.GameObjects.Particles
  .ParticleEmitter {
  soundFX:
  ...
  constructor(scene: Phaser.Scene, x: number, y: number) {
    super(scene, x, y, "lightning_particle", {
    ...
    this.soundFX = this.scene.sound.add("recharge", {
      loop: true,
      volume: 0.5,
    });
  ...
  updateDistance(playerX: number, playerY: number) {
    ...
    let pan = Phaser.Math.Clamp(
      (this.soundPosition.x - playerPos.x) / maxDistance,
      -1,
      1
    );
