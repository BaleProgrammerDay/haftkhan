...
export class Barghman extends Scene {
  ...
  create() {
    ...
    const { isPowerActive } = store.getState().general;

    this.powerGenerator = new PowerGenerator(this, width, height);
    this.powerGenerator.setY(
      this.powerGenerator.y - this.powerGenerator.height / 4
    );
    this.powerGenerator.setX(
      this.powerGenerator.x - this.powerGenerator.width / 4
    );
    ...
    if (!isPowerActive) this.wrench = new Wrench(this, 40, height); // Position at ground level with bottom origin
  ...
  private handleWrenchInteraction() {
    ...
    const distanceWithGenerator = Phaser.Math.Distance.Between(
      this.rostam.x,
      this.rostam.y,
      this.powerGenerator.x,
      this.powerGenerator.y
    );

    if (
      distanceWithGenerator < 340 &&
      distance < 60 &&
      !this.wrench.isAttached
    ) {
      if (this.eKey.isDown) {
    ...
    if (distance < 50 && this.hKey.isDown) {
      ...
      this.interactionHelper.setPromptActive(
        "holdh",
        distanceWithGenerator > 340 && distance < 50
      );
    }
  }
